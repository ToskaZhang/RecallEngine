...
class RecallEngineConfig:
    ...
    def from_env(cls) -> 'RecallEngineConfig':
        return cls(
            ...
            enable_adaptive_retrieval=os.getenv('CONTEXT_ADAPTIVE', 'true').lower() == 'true',
        )
...
class RecallEngine:
    ...
    def __init__(self, config: Optional[RecallEngineConfig] = None,
        ...
        self._stats = {"total_calls": 0, "memory_hits_avg": 0.0, "reroute_count": 0, "fallback_count": 0}
    ...
    def route_and_execute(self, user_input: str, session_id: Optional[str] = None,
                          inject_memory: bool = True, auto_archive: Optional[bool] = None) -> Dict[str, Any]:
        """完整的路由 + 记忆增强 + 执行流程"""
        start_time = time.perf_counter()
        sid = session_id or self._session_id
        auto_archive = auto_archive if auto_archive is not None else self.config.auto_archive
        ...
        if self.config.parallel_execution and need_retrieval:
            ...
            search_future = self._executor.submit(self._safe_search, user_input, sid, self.config.memory_top_k)
            route_result = route_future.result()
            memory_hits = search_future.result() or []
        else:
            route_result = self._safe_route(user_input)
            if need_retrieval:
                memory_hits = self._safe_search(user_input, sid, self.config.memory_top_k) or []
        
        if route_result is None:
            return self._build_error("未匹配到任何技能", start_time)
        
        # 3. 解包
        skill_name = getattr(route_result, 'skill_name', None) or getattr(route_result, 'name', None)
        confidence = getattr(route_result, 'confidence', 0.0)
        parameters = getattr(route_result, 'parameters', {}).copy() if hasattr(route_result, 'parameters') else {}
        ...
        # 4. 过滤记忆
        if memory_hits:
            memory_hits = self._filter_memory_hits(memory_hits, self.config.memory_min_score)
        
        memory_context = ""
        if memory_hits:
            memory_context = self._truncate_text(self._format_memory_context(memory_hits), self.config.max_memory_chars)
        
        # 5. 二次路由
        rerouted = False
        if memory_context and confidence < self.config.min_confidence_threshold:
            new_result = self._reroute_with_memory(user_input, route_result, memory_context)
            if new_result is not None:
                ...
                if new_conf > confidence + self.config.reroute_boost_threshold:
                    ...
                    confidence = new_conf
                    parameters = getattr(route_result, 'parameters', {}).copy() or parameters
                    rerouted = True
        ...
        if isinstance(execution_result, str):
            self._conversation_history.append({"role": "assistant", "content": execution_result[:500]})
        
        self._stats["memory_hits_avg"] = self._stats["memory_hits_avg"] * 0.9 + len(memory_hits) * 0.1
        
        return {
            "skill_name": skill_name, "confidence": confidence, "parameters": parameters,
            "memory_context": memory_context, "memory_hits": memory_hits,
            "execution_result": execution_result, "archived": archived,
            "rerouted": rerouted, "elapsed_ms": (time.perf_counter() - start_time) * 1000,
        }
    
    # ---------- 独立记忆操作 ----------
    def search_memory(self, query: str, top_k: Optional[int] = None, session_id: Optional[str] = None) -> List[Dict]:
        return self._safe_search(query, session_id or self._session_id, top_k) or []
    
    def archive_message(self, role: str, text: str, session_id: Optional[str] = None) -> bool:
        return self._memory_store.archive(role, text, session_id or self._session_id)
    
    def fetch_context(self, query: str, max_tokens: int = 500, session_id: Optional[str] = None) -> str:
        try:
            return self._memory_store.fetch_context(query, max_tokens, session_id or self._session_id)
        except Exception:
            return ""
    ...
    def _need_memory_retrieval(self, user_input: str) -> bool:
        ...
        skip = {"你好", "嗨", "hello", "hi", "谢谢", "感谢", "thanks", "是", "否", "对", "不",
                "继续", "开始", "结束", "停", "好", "ok", "好的", "嗯"}
        if stripped in skip:
            return False
        if len(stripped) < 3:
            return False
    ...
    def _filter_memory_hits(self, hits: List[Dict], min_score: float) -> List[Dict]:
        ...
        for h in filtered:
            text = h.get('text', '')
            if text and (text not in seen or h.get('score', 0) > seen[text].get('score', 0)):
                seen[text] = h
    ...
    def _format_memory_context(self, hits: List[Dict]) -> str:
        ...
        for i, h in enumerate(hits, 1):
            ts = datetime.fromtimestamp(h.get('timestamp', 0)).strftime("%H:%M") if h.get('timestamp') else ""
            lines.append(f"{i}. [{h.get('role', '?')}] {h.get('text', '')} (相关度: {h.get('score', 0):.2f}{', ' + ts if ts else ''})")
        return "\n".join(lines)
    
    def _truncate_text(self, text: str, max_chars: int) -> str:
        return text[:max_chars] + "...(已截断)" if len(text) > max_chars else text
    
    def _reroute_with_memory(self, user_input: str, original_result, memory_context: str):
        try:
            ...
            return new_result if new_conf > orig_conf + self.config.reroute_boost_threshold else None
        except Exception:
            return None
    
    def _to_skill_call(self, route_result, parameters: Dict) -> Any:
        if hasattr(route_result, 'execution_type') and hasattr(route_result, 'execution_target'):
            if SkillCall:
                return SkillCall(
    ...
    def _safe_search(self, query: str, session_id: str, top_k: Optional[int] = None) -> List[Dict]:
        try:
    ...
    def _safe_execute(self, call, skill_name: str) -> Union[str, Dict]:
        if self._execute_skill is None:
            return {"error": "执行器不可用，请安装 intent_classifier", "skill": skill_name}
        try:
    ...
    def _safe_archive(self, user_input: str, execution_result, session_id: str) -> bool:
        try:
            ...
            if isinstance(execution_result, str):
                self._memory_store.archive("assistant", execution_result[:500], session_id)
            elif isinstance(execution_result, dict) and "error" not in execution_result:
                self._memory_store.archive("assistant", json.dumps(execution_result)[:500], session_id)
            return True
        except Exception:
            return False
    
    def _build_error(self, msg: str, start_time: float) -> Dict:
        return {"skill_name": None, "confidence": 0.0, "parameters": {}, "memory_context": "",
                "memory_hits": [], "execution_result": {"error": msg}, "archived": False,
                "rerouted": False, "elapsed_ms": (time.perf_counter() - start_time) * 1000}
...
def recall_with_memory(user_input: str, session_id: Optional[str] = None, **kwargs) -> Dict[str, Any]:
    """快捷函数：带记忆增强的路由 + 执行"""
...
if __name__ == "__main__":
    ...
    for q in ["Docker 权限问题", "上次那个问题", "你好", "详细说一下"]:
        ...
        print(f"  记忆: {len(result['memory_hits'])}条 | 耗时: {result['elapsed_ms']:.1f}ms")
