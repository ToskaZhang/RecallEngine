...
def _import_with_fallback(
    module_name: str, fallback_paths: List[str], attrs: List[str]
):
    """从多个候选路径导入模块并获取指定属性"""
...
class ArchiverMemoryStore(BaseMemoryStore):
    ...
    def fetch_context(
        ...
        return self.archiver.fetch_relevant(
            query, max_tokens=max_tokens, session_id=session_id
        )
